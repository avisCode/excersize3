/*
File: bus.h
	Description : This is the header file of bus.cpp
...
Bus* busCtor(int, int = MAX_SEATS); // Create a bus with id and given number of seats
...
void busLine(Bus &, int busLine = 0);
...
Bus* getBus(int);   // Returns pointer to a bus object with given id, nullptr if the bus
					// does not exist
void delBus(uint);   // Removes a bus from the fleet and destroys it (frees its memory),
